...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        if static_kv:
            attn_probs = attn_probs.view(bsz // beam_size, beam_size, self.num_heads, tgt_len, src_len)
            # v = v.expand(bsz // beam_size, beam_size, self.num_heads, src_len, self.head_dim).reshape(-1, src_len, self.head_dim)
            attn_output = torch.einsum("bmhts,bnhsd->bmhtd", attn_probs, v).reshape(-1, tgt_len, self.head_dim)
        else:
    ...
    def _use_saved_state2(self, k, v, saved_state, key_padding_mask, static_kv, bsz):
        # saved states are stored with shape (bsz, num_heads, seq_len, head_dim)
        if "prev_key" in saved_state:
            _prev_key = saved_state["prev_key"]
            assert _prev_key is not None
